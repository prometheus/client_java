...
public class JettyStatisticsCollector extends Collector {
  ...
  private static MetricFamilySamples.Sample buildStatusSample(String name, String status,
      double value) {
  ...
  public List<MetricFamilySamples> collect() {
    return Arrays.asList(
        ...
        buildGauge("jetty_requests_active_max",
        ...
        buildGauge("jetty_request_time_seconds_total", "Total time spent in all request handling",
            statisticsHandler.getRequestTimeTotal() / 1000.0),
        buildGauge("jetty_dispatched_total", "Number of dispatches",
        ...
        buildGauge("jetty_dispatched_active_max",
        ...
        buildGauge("jetty_dispatched_time_seconds_total", "Total time spent in dispatch handling",
            statisticsHandler.getDispatchedTimeTotal() / 1000.0),
        buildGauge("jetty_async_requests_total", "Total number of async requests",
            statisticsHandler.getAsyncRequests()),
        buildGauge("jetty_async_requests_waiting", "Currently waiting async requests",
            statisticsHandler.getAsyncRequestsWaiting()),
        buildGauge("jetty_async_requests_waiting_max", "Maximum number of waiting async requests",
            statisticsHandler.getAsyncRequestsWaitingMax()),
        buildGauge("jetty_async_dispatches_total",
            "Number of requested that have been asynchronously dispatched",
            statisticsHandler.getAsyncDispatches()),
        buildGauge("jetty_expires_total", "Number of async requests requests that have expired",
        ...
        buildGauge("jetty_responses_bytes_total", "Total number of bytes across all responses",
