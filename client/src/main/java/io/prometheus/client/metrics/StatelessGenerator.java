...
/**
 * <p>
 * {@link StatelessGenerator} provides a simple means for label-oriented metric
 ...
 * </p>
 * 
 * @param <V> The {@link StatelessGenerator.Vector} type name that is
 *        responsible for representing a given dimension.
 * 
 * @author matt.proud@gmail.com (Matt T. Proud)
 */
abstract class StatelessGenerator<V extends StatelessGenerator.Vector> implements Metric {
  protected final Map<Map<String, String>, V> vectors = new MapMaker().makeMap();

  @Override
  public void ResetAll() {
    for (final Map<String, String> key : vectors.keySet()) {
      final V vector = vectors.get(key);
      vector.reset();
    }
   ...
   */
  protected static interface Vector<T> {
    /**
