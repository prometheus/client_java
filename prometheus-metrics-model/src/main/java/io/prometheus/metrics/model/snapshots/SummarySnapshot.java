...
public final class SummarySnapshot extends MetricSnapshot {
  ...
  public static final class SummaryDataPointSnapshot extends DistributionDataPointSnapshot {
    ...
    /**
     * To create a new {@link SummaryDataPointSnapshot}, you can either call the constructor
     * directly or use the Builder with {@link SummaryDataPointSnapshot#builder()}.
     *
     ...
     * @param quantiles must not be {@code null}. Use {@link Quantiles#EMPTY} if there are no
     *     quantiles.
     * @param labels must not be {@code null}. Use {@link Labels#EMPTY} if there are no labels.
     * @param exemplars must not be {@code null}. Use {@link Exemplars#EMPTY} if there are no
     *     exemplars.
     * @param createdTimestampMillis timestamp (as in {@link System#currentTimeMillis()}) when this
     *     summary data (this specific set of labels) was created. Note that this refers to the
     *     creation of the timeseries, not the creation of the snapshot. The created timestamp
     *     optional. Use {@code 0L} if there is no created timestamp.
     */
    public SummaryDataPointSnapshot(
        long count,
        double sum,
        Quantiles quantiles,
        Labels labels,
    ...
    public static class Builder extends DistributionDataPointSnapshot.Builder<Builder> {
      ...
      public SummaryDataPointSnapshot build() {
        return new SummaryDataPointSnapshot(
            count,
            sum,
            quantiles,
            labels,
